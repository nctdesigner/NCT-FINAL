...
const getBanners = async () => {
  try {
  const response = await axios.get("http://localhost:1337/api/bAuth/fetch", {
    headers: {
...
const getBanner = cache(async slug => {
  try {
  const response = await axios.get("http://localhost:1337/api/bAuth/get", {
    headers: {
      "serverPass": process.env.NEXT_PUBLIC_SERVER_PASSWORD,
      "slug": slug
    }
...
const createBanner = async (name, title, subtitle, redirectText, redirectUrl, image) => {
  ...
  try {
    ...
    const response = await axios.post("http://localhost:1337/api/bAuth/create", data, {
      headers: {
        "serverPass": process.env.NEXT_PUBLIC_SERVER_PASSWORD
      }
    });
...
const updateBanners = async (name, title, subtitle, redirectText, redirectUrl, image, bannerId) => {
  ...
  try {
    ...
    const response = await axios.post("http://localhost:1337/api/bAuth/update", data, {
      headers: {
        "serverPass": process.env.NEXT_PUBLIC_SERVER_PASSWORD,
        "id": bannerId
      }
...
const deleteBanners = async (bannerId) => {
  try {
    const response = await axios.get("http://localhost:1337/api/bAuth/delete", {
      headers: {
        "serverPass": process.env.NEXT_PUBLIC_SERVER_PASSWORD,
        "id": bannerId
      }
